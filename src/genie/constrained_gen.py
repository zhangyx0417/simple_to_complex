...
class BartConstrainedGen(PreTrainedModel):
    def __init__(self, config, tokenizer):
        ...
        if os.path.exists("best_temp.npy"):
            self.best_temp = np.load("best_temp.npy").tolist()
            print("------Temperature file found, logits will be scaled with T=" + str(self.best_temp) + " for simple-to-complex ordering------")
        else:
            self.best_temp = 1.0
            print("------Temperature file not found, logits will not be scaled------")
    ...
    def prepare_inputs_for_generation(
            self, decoder_input_ids, past, attention_mask, use_cache, encoder_outputs, input_embeds, encoder_input_ids,
            **kwargs):
        return {
    ...
    # # this is a simplified generate class for the pointer generator taken from
    # https://github.com/huggingface/transformers/blob/v3.1.0/src/transformers/generation_utils.py
    @torch.no_grad()
    def generate(
            self,
        ...
        output, id2prob = self._generate_no_beam_search(
            input_ids,
            cur_len=cur_len,
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_token_logits /= self.best_temp
                # Greedy decoding
                probs = F.softmax(next_token_logits, dim=-1)
                next_token = torch.argmax(probs, dim=-1)
                top2probs, _ = torch.topk(probs, 2, dim=-1)
                top2probs, idx = top2probs.squeeze().tolist(), next_token.item()
                id2prob.append([idx, top2probs[0]])
